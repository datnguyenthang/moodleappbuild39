 ...
 */
@IonicPage({ segment: 'addon-mandatory-my-mandatory' })
@Component({
    selector: 'page-addon-mandatory-my-mandatory',
    templateUrl: 'my-mandatory.html',
})
export class AddonMandatoryMyMandatoryPage implements OnDestroy {
    ...
    protected fetchUserCourses(): Promise<any> {
        return this.landingProvider.getUserCourses(null, null, AddonLandingProvider.TYPE_MANDATORY_TRAINING).then((courses) => {
            const promises = [],
                courseIds = courses.map((course) => {
                return course.id;
            });
